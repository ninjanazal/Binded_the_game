...
public class AikeBehavior : MonoBehaviour
{
   ...
   private void InputManager()
   {
      ...
      if (Input.GetAxis("Horizontal") != 0f)
      {
         ...
         target_direction_ += Quaternion.Euler(0f, 90f, 0f)
            * Vector3.ProjectOnPlane(char_system_.ProjectDirection(), char_transform_.up) *
   ...
   private void VelocityControl()
   {
      ...
      if (char_acceleration_ == 0 && char_speed_ > 0 && GroundedCheck())
   ...
   private void AjustCharRotation()
   {
      ...
      Quaternion target_rotation_;  // rotaçao alvo
         ...
         char_system_.maxFloorDistance * 2f, char_system_.GroundMask))
      {
         ...
         target_rotation_ = (Quaternion.LookRotation(Vector3.ProjectOnPlane(
            char_transform_.forward, hit.normal), hit.normal));
      }
      else
